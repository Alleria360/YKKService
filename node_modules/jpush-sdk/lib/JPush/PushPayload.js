...
function android(alert, title, builder_id, extras, priority, category, style, value) {
    if (alert != null) {
...
function setSmsMessage(content, delayTime) {
  ...
  this.payload = JUtil.extend(this.payload, {'sms_message': smsMessage})
  return this
}
...
function setSingleSchedule(date) {
    ...
    var single = {'time': date};
    this.trigger = JUtil.extend(this.trigger, {'single': single});
    return this;
}
...
function setPeriodicalSchedule(start, end, time, timeUnit, frequency, point) {
    var periodical = {
        'start'    : start, 'end': end, 'time': time, 'time_unit': timeUnit,
        'frequency': frequency, 'point': point
    };
    this.trigger = JUtil.extend(this.trigger, {'periodical': periodical});
    return this;
}
...
function setSchedule(name, enabled, callback) {
    ...
    validate(this.payload);
    this.payload.options = JUtil.extend({'sendno': generateSendno(), 'apns_production': false},
        this.payload.options);
    var body = {'name': name, 'enabled': enabled, 'trigger': this.trigger, 'push': this.payload};
...
function updateSchedule(scheduleId, name, enabled, callback) {
    ...
    if (name != null) {
        ...
        body = JUtil.extend(body, {'name': name});
    }
    if (enabled != null) {
        if (typeof enabled != 'boolean') {
            throw new JError.InvalidArgumentError('enabled must be set to boolean.');
        }
        body = JUtil.extend(body, {'enabled': enabled});
    }
    if (!JUtil.isEmptyObject(this.trigger)) {
        body = JUtil.extend(body, {'trigger': this.trigger});
    }
    if (!JUtil.isEmptyObject(this.payload)) {
        validate(this.payload)
        this.payload.options = JUtil.extend({'sendno': generateSendno(), 'apns_production': false},
            this.payload.options);
        body = JUtil.extend(body, {'push': this.payload});
    }
