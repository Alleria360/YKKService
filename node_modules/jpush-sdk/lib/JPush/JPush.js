...
function setMobile (registrationId, mobileNumber) {
  var header = {
    'User-Agent': USER_AGENT,
    'Connection': 'Keep-Alive',
    'Charset': 'UTF-8',
...
function setSchedule (payload, callback) {
  ...
  return _request(SCHEDULE_API_URL, payload, header, {
    user: this.appkey,
    pass: this.masterSecret
  }, 'POST', 1, this.retryTimes, this.isDebug, callback)
}
...
function updateSchedule (scheduleId, payload, callback) {
  ...
  return _request(url, payload, header, {
    user: this.appkey,
    pass: this.masterSecret
  ...
  var _callback = function (err, res, body) {
    if (err) {
      ...
      if (times < maxTryTimes) {
        return _request(url, body, headers, auth, method, times + 1, maxTryTimes, isDebug, callback)
      } else {
